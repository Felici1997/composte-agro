...
import { Package, PlusCircle, ShoppingBag, MapPin, User, Tag, TrendingUp, Clock, Eye, ChevronRight, Sprout, Store } from 'lucide-react'
...
export default function VendeurHome() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (session?.user) {
        const listings = await fetchListingsByCategoryIds([1, 2, 6, 7, 42])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-emerald-700 to-green-800 rounded-3xl p-8 md:p-10 mb-8 text-white shadow-xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border border-slate-200 rounded-xl transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 hover:border-agrishop-200 p-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0"><Package size={20} className="text-emerald-600" /></div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 hover:border-agrishop-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold text-slate-800">0</p><p className="text-xs text-slate-500">Vues totales</p></div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-xs bg-emerald-50 text-emerald-600 font-medium px-2.5 py-0.5 rounded-full">{demandes.length} nouvelle{demandes.length > 1 ? 's' : ''}</span>
        </div>
        {loading ? <div className="text-center py-12 text-slate-400">Chargement...</div> : demandes.length > 0 ? (
          <div className="space-y-2">
            {demandes.slice(0, 5).map(d => (
              <Link key={d._key || d.id} href={'/ad/' + d.id} className="block bg-white border border-slate-200 rounded-xl p-4 hover:border-emerald-200 hover:shadow-md transition">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-slate-800 truncate">{d.title}</p>
                    <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-slate-500">
                      {d._category && <span className="inline-flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-full"><Tag size={10} /> {d._category.nom}</span>}
                  ...
                  <div className="shrink-0">
                    <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg hover:bg-emerald-100 transition">Voir <ChevronRight size={14} /></span>
                  </div>
                </div>
              </Link>
            ))}
