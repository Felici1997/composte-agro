...
CSV_CONFIG = {
    ...
    "gsap": {"file": "motion.csv", "search_cols": ["Category", "Intensity Tier", "Keywords", "Trigger"], "output_cols": ["Category", "Intensity Tier", "Trigger", "Duration", "Easing", "GSAP Snippet", "Framework Notes", "Do", "Don't", "Performance Notes"]},
...
_STOPWORDS = {"to", "in", "on", "at", "is", "of", "by", "or", "an", "if", "no", "so", "do", "be", "we", "it", "as", "the", "and", "for", "are", "was"}

_SYNONYMS = {"e-commerce": "ecommerce", "dark-mode": "dark", "darkmode": "dark", "light-mode": "light", "lightmode": "light", "a11y": "accessibility", "nav": "navigation", "sign-up": "signup", "log-in": "login", "colour": "color", "colours": "colors", "customisation": "customization", "organisation": "organization", "behaviour": "behavior", "ux/ui": "ux ui"}
...
class BM25:
    def __init__(self, k1=1.5, b=0.75):
        self.k1 = k1; self.b = b; self.corpus = []; self.doc_lengths = []; self.avgdl = 0
        self.idf = {}; self.doc_freqs = defaultdict(int); self.N = 0; self._term_freqs = []
    ...
    def fit(self, documents):
        ...
        for doc in self.corpus:
            tf = defaultdict(int)
            for word in doc: tf[word] += 1
            self._term_freqs.append(tf)
            for word in tf: self.doc_freqs[word] += 1
    ...
    for term in bm25.vocabulary():
        for qt in query_tokens:
            if term.startswith(qt[:3]) or qt.startswith(term[:3]):
                candidates.append(term); break
    seen = set(); ordered = []
    for term in candidates:
        if term not in seen: seen.add(term); ordered.append(term)
    return ordered[:limit]

_DOMAIN_KEYWORDS = None

def _load_product_keywords():
    seed = ["saas", "ecommerce", "e-commerce", "fintech", "healthcare", "gaming", "portfolio", "crypto", "dashboard", "fitness", "marketplace"]
    filepath = DATA_DIR / CSV_CONFIG["product"]["file"]
    if not filepath.exists(): return seed
    try:
    ...
    for row in rows:
        ...
        for kw in re.split(r"[,;]", raw):
            kw = kw.strip().lower()
            if kw and len(kw) >= 3: keywords.add(kw)
    return sorted(keywords, key=len, reverse=True)

def _domain_keywords():
    global _DOMAIN_KEYWORDS
    if _DOMAIN_KEYWORDS is not None: return _DOMAIN_KEYWORDS
    _DOMAIN_KEYWORDS = {
...
_DOMAIN_TIEBREAK_ORDER = ["ux", "product", "style", "color", "typography", "google-fonts", "chart", "landing", "icons", "gsap", "react", "web"]
...
def detect_domain(query, return_scores=False):
    ...
    for domain, keywords in domain_keywords.items():
        ...
        scores[domain] = total
    ranked = sorted(scores.items(), key=lambda item: (item[1], -_DOMAIN_TIEBREAK_ORDER.index(item[0]) if item[0] in _DOMAIN_TIEBREAK_ORDER else -999), reverse=True)
...
def search(query, domain=None, max_results=MAX_RESULTS):
    ...
    if auto_detected:
        ...
        if runner_up: out["runner_up_domain"] = runner_up
    if not results:
...
def search_stack(query, stack, max_results=MAX_RESULTS):
    ...
    out = {"domain": "stack", "stack": stack, "query": query, "file": STACK_CONFIG[stack]["file"], "count": len(results), "results": results}
    if not results:
        out["suggestions"] = _suggest_terms(bm25, query)
    return out
