...
import { cached, clearCache, clearCacheByPrefix, cacheKey, getPrefixes } from '@/lib/cache'
...
export async function fetchCategories() {
  return cached(P.categories, async () => {
    try {
...
export async function fetchAllProducts() {
  return cached(P.products, async () => {
    try {
...
export async function fetchAllListings() {
  return cached(P.listings, async () => {
    try {
      const { data, error } = await fetchWithTimeout(
      ...
      const enriched = await enrichWithProfiles(data || [], l => l.client_id)
...
export async function fetchAllServices() {
  return cached(P.services, async () => {
    try {
      const { data, error } = await fetchWithTimeout(
...
export async function fetchAdById(id) {
  ...
  return cached(cacheKey(P.adById, strId), async () => {
    try {
      ...
      if (!error && data) {
        const profile = await safeGetProfile(data.client_id)
...
export async function fetchProfile(userId) {
  if (!userId) return null
  return cached(cacheKey(P.profile, userId), () => safeGetProfile(userId), 120000)
}

export async function fetchListingsByCategoryIds(categoryIds) {
  if (!categoryIds || categoryIds.length === 0) return []
  const key = cacheKey(P.listingsByCat, [...categoryIds].sort().join(','))
...
export async function fetchUserListings(userId) {
  if (!userId) return []
  return cached(cacheKey(P.userListings, userId), async () => {
    try {
      const { data, error } = await fetchWithTimeout(
        supabase.from('listings').select('*').eq('client_id', userId).order('created_at', { ascending: false }).limit(50)
      )
...
export async function fetchUserProducts(userId) {
  if (!userId) return []
  return cached(cacheKey(P.userProducts, userId), async () => {
    try {
      const { data, error } = await fetchWithTimeout(
...
export async function fetchUserServices(userId) {
  if (!userId) return []
  return cached(cacheKey(P.userServices, userId), async () => {
    try {
      const { data, error } = await fetchWithTimeout(
...
export async function invalidateAdsCache(type) {
  if (type === 'product') {
    await Promise.all([
      clearCache(P.allAds),
      clearCache(P.products),
      clearCacheByPrefix(P.adById),
      clearCacheByPrefix(P.userProducts),
    ])
  } else if (type === 'service') {
    await Promise.all([
      clearCache(P.allAds),
      clearCache(P.services),
      clearCacheByPrefix(P.adById),
      clearCacheByPrefix(P.userServices),
    ])
  } else {
    await Promise.all([
      clearCache(P.allAds),
      clearCache(P.listings),
      clearCacheByPrefix(P.adById),
      clearCacheByPrefix(P.userListings),
      clearCacheByPrefix(P.listingsByCat),
    ])
  }
}
